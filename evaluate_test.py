...
def create_big_output(model, transformed_image, model_input_size=(400, 400), step=104):
    ...
    for start_h in range(0, out_shape[-2] - model_input_size[-2], step):
        for start_w in range(0, out_shape[-1] - model_input_size[-1], step):
...
def generate_test_output(model, transform, test_path, out_path):
    ...
    for test_dir in test_dirs[:1]:
        ...
        df = export_to_df(prediction, test_id=test_dir.split("test_")[0])
        dataframes.append(df)
        
        out_file = os.path.join(out_path, test_dir + ".png")
        cv2.imwrite(out_file, prediction.numpy())  # unsqueeze ? 
...
if __name__ == '__main__':
    ...
    load_path = "model/trained_models/WholeModel_2_0.001_10_True_torch.float32cv.pth"
